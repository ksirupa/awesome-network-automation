...
- [Anuta Networks](https://www.anutanetworks.com/) - Anuta ATOM delivers analytics and closed-loop automation for greenfield and brownfield deployments. ATOM is an intent-driven platform with YANG service models for multiple domains such as Branch/CPE, Data Center, Virtual MPLS, Carrier Core Networks and Cloud Interconnects. ATOM has built-in YANG device models for PNFs and VNFs from 45+ industry leading vendors supporting hundreds of L2-L7 network functions using CLI, NETCONF, YANG, REST and SNMP. See the complete [list](http://www.anutanetworks.com/managed-devices/).
